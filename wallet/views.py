...
from wallet.models import PaymentWallet
...
def withdrawMoney(request):
    if request.method == "POST":
        ...
        if int(amount) > request.user.cash:
            ...
            PaymentWalletData = PaymentWallet(user=request.user, cash=amount, typePayment=False, cardNumber=cardNumber,
                                              name=name)
            context = {
...
def verify(request, amount):
    ...
    if request.GET.get('Status') == 'OK':
        ...
        if result.Status == 100:
            ...
            paymentResult = PaymentWallet.objects.create(user=request.user, cash=amount, RefID=str(result.RefID))
            paymentResult.save()
            return redirect("/wallet/")
