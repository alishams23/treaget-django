...
from wallet.models import PaymentWalletData
...
def withdrawMoney(request):
    if request.method == "POST":
        ...
        if int(amount) > request.user.cash:
            ...
            PaymentWallet = PaymentWalletData(user=request.user, cash=amount, typePayment=False, cardNumber=cardNumber,
                                              name=name)
            context = {
...
def verify(request, amount):
    ...
    if request.GET.get('Status') == 'OK':
        ...
        if result.Status == 100:
            ...
            paymentResult = PaymentWalletData.objects.create(user=request.user, cash=amount, RefID=str(result.RefID))
            paymentResult.save()
            return redirect("/wallet/")
